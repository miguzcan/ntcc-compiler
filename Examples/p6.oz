...
define 
   ...
   S = fun{$ parms( X1 Y1 Z1 W1)}
	  when(proc{$ Root} Root.current.X1  >:  0  Root.residual.Y1  >:  0    end
	       next(
		  par( 
		     tell(proc{$ Root} Root.current.X1  =:  1    end)
		     tell(proc{$ Root} Root.current.Y1  =:  1    end)
		     tell(proc{$ Root} Root.current.Z1  =:  Root.residual.Z1  +  1    end)
		     tell(proc{$ Root} Root.current.W1  =:  Root.residual.W1  -  1    end)
		     ) 
		  ) 
	      ) 
       end  
   ...
   WhenEver = fun{$ parms( Alpha2 Rho2)}
